...
if  __name__ == '__main__':
    ...
    checkpoint_file=tf.train.latest_checkpoint("/home/goerlab/Welder_detection/code/20180210/model/20180208_40/")
    ...
    csv_dir="./model/20180208_40/"
    csvfile=open(csv_dir+"0209spk_img3_0.csv","wb")
    writer=csv.writer(csvfile)
    writer.writerow(["labels","prediction","confidence_score","file_name"])
    image_dir = "/media/goerlab/My Passport/Elements/20180209@@@@@/img3/0/"
    cnt=0
    print("here")
    for root,dirs,files in os.walk(image_dir):
        ...
        for i in files:
            ...
            if "Good" in image_file:
                real_label = 0
            if "NoWeld" in image_file:
                    real_label = 1
            if "NoWire" in image_file:
                    real_label = 2
            if "ExtraWire" in image_file:
                    real_label = 3
            if "Offset" in image_file:
                    real_label = 4
            if "WireGap" in image_file:
                    real_label = 5
            if "UnSure" in image_file:
                real_label=6
