...
class vgg16:
    ...
    def fc_layers(self):
        ...
        with tf.variable_scope('fc-new') as scope:
            fc3w = tf.get_variable('W', [512*512, 13], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
    ...
    def load_initial_weights(self, session):
        ...
        with tf.variable_scope('fc-new', reuse = True):
            ...
            '''
            last_layer_weights = np.load('last_layers_epoch_4.npz')
            print('Last layer weights: last_layers_epoch_4.npz')
            var = tf.get_variable('W', trainable = True)
            print('Adding weights to',var.name)
...
if __name__ == '__main__':
    ...
    '''
    train_data = h5py.File('/home/goerlab/Bilinear-CNN-TensorFlow/dataset/solder_pad_regionA/new_val_448.h5', 'r')
    val_data = h5py.File('/home/goerlab/Bilinear-CNN-TensorFlow/dataset/solder_pad_regionA/new_val_448.h5', 'r')
    ...
    #for i in range(total_val_batch):  #####This is origin
    for i in range(2):
    ...
    for epoch in range(2):   ####queenie edited, origin: 100########################
        ...
        if epoch % 5 == 0:
            ...
            #tf.layers.files.save_npz(vgg16.all_params,name="")
            # all_layer_weights = []
            # # for v in vgg.parameters:
            # #    print(v)
            # print("collect_list:")
            # collect_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
            # for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES):
            #     print('Printing Trainable Variables :', sess.run(v).shape)
            #     all_layer_weights.append(sess.run(v))

            fine_tune_model = "./all_layers2_epoch_" + str(epoch)
            # all_layer_weights.append(sess.run(v))
